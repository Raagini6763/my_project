...
const items = [['Home', 'home', '/dashboard'], ['Stories', 'menu-book', '/stories'], ['Upload', 'ios-share', '/upload'], ['Campaigns', 'campaign', '/campaigns'], ['Action', 'bolt', '/action']] as const;

export function UserBottomNav() {
  const pathname = usePathname();
  return <View style={styles.nav}>{items.map(([label, icon, route]) => {
    const active = pathname === route || (route === '/dashboard' && pathname === '/');
    return <Pressable key={route} style={styles.item} onPress={() => router.push(route)}><MaterialIcons name={icon} size={25} color={active ? '#087D97' : '#5B6470'} /><Text style={[styles.label, active && styles.active]}>{label}</Text></Pressable>;
  })}</View>;
}
